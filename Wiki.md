...
Set up a "Pairing" listener, and the result will be returned through the data stream.
...
widget.blePlugin.createBondEveStm.listen(
  (int event) {
    if (!mounted) return;
    setState(() {
      _bondState = event; /// Return to pairing state int
    });
  },
),
```

Carry out the watch pairing process:

1. The parameter byte[] is a 6-digit random number, which is randomly generated by the user and saved locally.
2. After passing the parameter, the corresponding pairing status can be obtained, as shown in the following table;
...
Special processing for Android devices (only applicable when the corresponding pairing code exists for some watches): 
If there is a firmware code on the firmware end of the watch, the following method needs to be used. Here, byte[] represents a 6-digit random number, isBond indicates whether it is the first connection, the status needs to be stored locally by yourself, and pairingCode is the pairing code corresponding to the watch; the returned data is the same as above.
